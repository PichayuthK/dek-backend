...
bnUtil.cardName=config.cardName;
bnUtil.connect(main);
...
function main(error){
    ...
    // #3 Build the query object
    var query = bnUtil.connection.buildQuery(statement);

        // #4 Execute the query
        return bnUtil.connection.query(query)//,{id:'CRAFT01'});
    .then((result)=>{
        console.log('Received card count:', result.length);
        if(result.length > 0) console.log(result[0].cardId);
